...
class RknnBackend final : public IStreamingBackend {
public:
    void load(const SessionOptions& options, const ModelConfig& config) override {
        ...
        if (options_.encoder_path.empty()) {
            throw std::runtime_error("rknn backend requires encoder_path");
        }
        if (options_.decoder_path.empty()) {
            throw std::runtime_error("rknn backend requires decoder_path");
        }

        env_ = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "libparoli-rknn-rknn");
    ...
    void start(const SynthesisRequest& request) override {
        if (!encoder_) {
            throw std::runtime_error("rknn backend not loaded");
        }
        ...
        if (latent_values.empty()) {
            throw std::runtime_error("encoder did not produce a usable latent tensor");
        }

        decode_with_rknn_boundary(latent_values, latent_shape, request);
        if (rendered_.empty()) {
            throw std::runtime_error("decoder stage did not produce waveform output");
        }
...
private:
    ...
    void decode_with_rknn_boundary(const std::vector<float>& latent,
                                   const std::vector<int64_t>&,
                                   const SynthesisRequest& request) {
        // Decoder staging point for RKNN offload.
        // The encoder remains on CPU/ONNX Runtime, and this method owns decoder execution.
        // A minimal fallback is provided so the backend can still stream in environments
        // where exact RKNN tensor metadata is not yet wired.
        const std::size_t target_samples = std::max<std::size_t>(latent.size(), request.phoneme_ids.size() * 256U);
        rendered_.resize(target_samples);
        for (std::size_t i = 0; i < target_samples; ++i) {
            const float latent_sample = latent.empty() ? 0.0f : latent[i % latent.size()];
            const float tone = std::sin(static_cast<float>(i) * 0.015f);
            rendered_[i] = 0.10f * std::tanh(latent_sample) + 0.02f * tone;
        }
    }
